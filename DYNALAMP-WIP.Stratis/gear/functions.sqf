...
fnc_save_gear = 
{
	...
	profileNameSpace setVariable[format["vas_gear_%1",_sel],[_edit,_primary,_launcher,_handgun,_magazines,_uniform,_vest,_backpack,_items,_primitems,_secitems,_handgunitems]];
...
fnc_load_gear =
{
	...
	if(isNil {_sel}) then {_sel = 0;};
	//hint format["%1", _sel];
	if(!isnil {profileNameSpace getVariable format["vas_gear_%1",_sel]}) then
	{
		_loadout = profileNameSpace getVariable format["vas_gear_%1",_sel];
		...
		for "_i" from 0 to 2 do
		{
			...
			{
				player addHandgunItem (_handguntems select _i);
			};
		};
	};
};
...
vas_loadonrespawn =
{
	...
	{
		if(!isnil {profileNameSpace getVariable format["vas_gear_%1",vas_loadout]}) then
		{
			_loadout = profileNameSpace getVariable format["vas_gear_%1",vas_loadout];
			_primary = _loadout select 1;
			_launcher = _loadout select 2;
			_handgun = _loadout select 3;
			_magazines = _loadout select 4;
...
fnc_gear_weapons = 
{
	private["_cfgweapons","_weapons","_cur_wep","_classname","_scope","_picture","_wep_type","_name"];
	_weapons = [];
