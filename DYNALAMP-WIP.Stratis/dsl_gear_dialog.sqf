...
fn_dsl_SelfView_Text_Headgear = {TEXT_HEADGEAR ctrlSetText "Headgear: " + (dsl_HatTypes select dsl_HeadGearIncrementer);}; 
fn_dsl_SelfView_Text_Face = {TEXT_FACE ctrlSetText "Face: " + (dsl_Heads select dsl_Face_Ind);};
fn_dsl_SelfView_Text_Vest = {TEXT_VEST ctrlSetText "Vest: " + (dsl_VestTypes select dsl_VestIncrementer);};
fn_dsl_SelfView_Text_Uniform = {TEXT_UNIFORM ctrlSetText "Uniform: " + (dsl_UniformTypes select dsl_UniformIncrementer);};
fn_dsl_SelfView_Text_Backpack = {TEXT_BACKPACK ctrlSetText "Backpack: " + (dsl_BackpackTypes select dsl_BackPackIncrementer)};
fn_dsl_SelfView_Text_Weapon = {TEXT_WEAPON ctrlSetText "Weapon: " + (dsl_WeaponListPrimaries select dsl_WeaponListPrimaries_ind);};
fn_dsl_SelfView_Text_Sidearm = {TEXT_SIDEARM ctrlSetText "Sidearm: " + (dsl_WeaponListPistols select dsl_WeaponListPistols_ind);};
fn_dsl_SelfView_Text_Optic = {TEXT_OPTIC ctrlSetText "Optic: " + (dsl_CurrentOpticTypes select dsl_OpticIndex);};
fn_dsl_SelfView_Text_Rail = {TEXT_RAIL ctrlSetText "Rail: " + (dsl_WeaponAccessoryTypes select dsl_AccessoryIndex)};
fn_dsl_SelfView_Text_Launcher = {TEXT_LAUNCHER ctrlSetText "Launcher: " + (dsl_Launchers select dsl_Launchers_ind)};
...
{
	copytoclipboard str([dsl_HatTypes select dsl_HeadGearIncrementer,
...
fn_dsl_SetUpSelfPortraitCam =
{
	...
	waitUntil 
	{
		...
		if (_inc == 1) then {_modDir = (_pdir + 30) + (180 * dsl_SelfCam_FrontOrBack); _delayTime = 5; _distance = .45;};
		if (_inc == 2) then {_modDir = (_pdir + 50) + (180 * dsl_SelfCam_FrontOrBack); if (dsl_SelfCam_FocusOn == "weapon") then {_modDir = (_pdir + 10)}; _delayTime = 2; _distance = .45;};
		if (_inc == 3) then {_modDir = (_pdir - 30) + (180 * dsl_SelfCam_FrontOrBack); _delayTime = 5;_distance = .45;};
		if (_inc == 4) then {_modDir = (_pdir - 50) + (180 * dsl_SelfCam_FrontOrBack);  if (dsl_SelfCam_FocusOn == "weapon") then {_modDir = (_pdir - 10)}; _delayTime = 2;_distance = .45;};
		_inc = _inc + 1; 
