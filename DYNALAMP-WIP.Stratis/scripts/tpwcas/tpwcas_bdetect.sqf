...

				{
					if( _x != _shooter ) then {
						...
						if( !(_x in _blacklist)  ) then {
							...
							//if( (vehicle _x == _x) && (local _x) && (lifestate _x == "ALIVE") ) then //ARMA3
							if( (vehicle _x == _x) && (local _x) && ((lifestate _x == "HEALTHY") || (lifestate _x == "INJURED")) ) then 
							{
								_blacklist set [ count _blacklist, _x];
								_update_blacklist = true;
...
tpwcas_fnc_log_benchmark =
{
	...
	_nul = [] spawn
	{
		...
		while { tpwcas_log_benchmark } do
		{
			...
			if( diag_fps < bdetect_stats_min_fps ) then { bdetect_stats_min_fps = diag_fps };
			diag_log text format["TIME: %1;FPS: %2 (min: %3);BULLETS: %4 (max: %5);S.DELAY: %6 (Min FPS: %7);FIRED: %8 (BLUE:%9 RED:%10);TRACKED: %11;DETECTED: %12;BLACKLISTED: %13;UNITS: %14;KILLED: %15",
			time,
			diag_fps,
			bdetect_stats_min_fps,
			_cnt,
